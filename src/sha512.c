...
/* FIPS 180-4 section 4.2.3 - first 64 bits of the fractional parts of the
...
static inline u64 big_sigma0(u64 x) {
    return rotr64(x, 28) ^ rotr64(x, 34) ^ rotr64(x, 39);
}
static inline u64 big_sigma1(u64 x) {
    return rotr64(x, 14) ^ rotr64(x, 18) ^ rotr64(x, 41);
}
static inline u64 small_sigma0(u64 x) {
    return rotr64(x, 1)  ^ rotr64(x, 8)  ^ (x >> 7);
}
static inline u64 small_sigma1(u64 x) {
...
static void compress_one(u64 state[8], const u8 block[128]) {
    u64 w[80];
    for (int i = 0; i < 16; i++) {
    ...
    for (int i = 0; i < 80; i++) {
        u64 t1 = h + big_sigma1(e) + Ch(e, f, g) + K[i] + w[i];
...
/* FIPS 180-4 section 5.3.5 - initial hash values for SHA-512:
...
static inline void add_bitlen(sha512_ctx* c, u64 bytes) {
    u64 add = bytes << 3;
    u64 carry_into_lo = (bytes >> 61); /* high 3 bits of bytes shift up */
    u64 old = c->bitlen_lo;
    c->bitlen_lo = old + add;
...
void sha512_update(sha512_ctx* c, const void* data, usize len) {
    ...
    add_bitlen(c, (u64)len);

    /* Drain a partial block first. */
    if (c->buf_len) {
        usize need = SHA512_BLOCK_LEN - c->buf_len;
        if (len < need) {
