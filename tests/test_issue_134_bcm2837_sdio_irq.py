...
armctrl = (root / "src" / "legacy_armctrl.c").read_text(encoding="utf-8")
...
assert "#define ARMCTRL_IRQ_PENDING1         0x04U" in legacy
assert "#define ARMCTRL_IRQ_PENDING2         0x08U" in legacy
assert "#define ARMCTRL_ENABLE_IRQS1         0x10U" in legacy
assert "#define ARMCTRL_ENABLE_IRQS2         0x14U" in legacy
assert "#define ARMCTRL_DISABLE_IRQS1        0x1CU" in legacy
assert "#define ARMCTRL_DISABLE_IRQS2        0x20U" in legacy
assert "{ LEGACY_ARMCTRL_INTID_SDIO1, LEGACY_ARMCTRL_BANK2, 30U, 0U }" in armctrl
assert "{ LEGACY_ARMCTRL_INTID_DWC2, LEGACY_ARMCTRL_BANK1, 9U, 1U }" in armctrl

route = body_after(legacy, "bool gic_legacy_route_gpu_core0(void)\n")
...
assert ack.index("QA7_LOCAL_IRQ_PENDING0") < ack.index("legacy_armctrl_select_pending")
assert "QA7_BIT_GPU_FAST" in ack
assert "legacy_armctrl_registered_mask" in ack
assert "legacy_armctrl_select_pending" in ack

enable = body_after(legacy, "void gic_enable_irq(u32 intid)\n")
disable = body_after(legacy, "void gic_disable_irq(u32 intid)\n")
assert "legacy_armctrl_registered" in enable
assert "enable_off" in enable
assert "disable_off" in disable
...
assert arm.index("irq_register(PIOS_WIFI_SDIO_IRQ") < arm.index("gic_legacy_register_gpu_irq(PIOS_WIFI_SDIO_IRQ)")
assert arm.index("gic_legacy_register_gpu_irq(PIOS_WIFI_SDIO_IRQ)") < arm.index("gic_legacy_route_gpu_core0()")
assert arm.index("gic_legacy_route_gpu_core0()") < arm.index("gic_enable_irq(PIOS_WIFI_SDIO_IRQ)")
