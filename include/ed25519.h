 ...
 *
 * PIOS scope. Variable-time sign and verify. No precomputed
 * base-point table. No SIMD. Does NOT yet reject small-order public
 * keys -- fine while PIOS only signs with its own cert and never
 ...
 */
#ifndef PIOS_ED25519_H
#define PIOS_ED25519_H
...
void ed25519_pubkey_from_seed(u8 pk[ED25519_PUBKEY_LEN],
                              const u8 seed[ED25519_SEED_LEN]);

void ed25519_sign(u8 sig[ED25519_SIG_LEN],
                  const u8* msg, usize msg_len,
                  const u8 seed[ED25519_SEED_LEN],
                  const u8 pk[ED25519_PUBKEY_LEN]);

int ed25519_verify(const u8 sig[ED25519_SIG_LEN],
                   const u8* msg, usize msg_len,
                   const u8 pk[ED25519_PUBKEY_LEN]);
