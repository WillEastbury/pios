...
set CFLAGS=-Wall -Wextra -Wno-unused-function -ffreestanding -nostdlib -nostartfiles -std=gnu11 -march=armv8-a -mgeneral-regs-only -mstrict-align -Iinclude -O2 -fno-builtin -DPIOS_PLATFORM=PIOS_PLATFORM_QEMU_VIRT
...
for %%f in (qemu_virt_walfs sd bcache walfs lru storage_layout) do (
    "%CC%" %CFLAGS% -c src\%%f.c -o build_qemu_walfs\%%f.o
    if errorlevel 1 exit /b 1
)
"%LD%" -T link_qemu_virt.ld -nostdlib -o qemu_walfs.elf build_qemu_walfs\qemu_virt_start.o build_qemu_walfs\qemu_virt_walfs.o build_qemu_walfs\sd.o build_qemu_walfs\bcache.o build_qemu_walfs\walfs.o build_qemu_walfs\lru.o build_qemu_walfs\storage_layout.o
if errorlevel 1 exit /b 1
